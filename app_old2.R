...
# Population defect profile 
      ...
      SampQtyRPIN <- DefectAssessment |>
        filter(Season == 2025,
               Orchard %in% input$Orchards) |> #c("Te Arai")) |> #
        inner_join(GraderBatch |>
                     filter(Season == 2025,
                            `Batch closed` == 1,
                            `Packing site` == "Te Ipu Packhouse (RO)") |>
    ...
    output$defectHeatMap <- renderPlot({
      ...
      if(nrow(GraderBatch |> 
        ...
        SampQtyHM <- DefectAssessment |>
          filter(Season == 2025,
                 #Orchard %in% c("Home Block", "Stock Roads")) |> 
                 Orchard %in% input$Orchards) |> 
          group_by(Orchard, `Production site`, `Management area`, GraderBatchID) |>
          summarise(SampleQty = max(SampleQty, na.rm=T),
                    .groups = "drop") |>
          summarise(SampleQty = sum(SampleQty))
        ...
        DA2025byPS |>  
          filter(#Orchard %in% c("Home Block", "Stock Roads"), 
                 Orchard %in% input$Orchards, 
                 Defect %in% top_ten) |>
